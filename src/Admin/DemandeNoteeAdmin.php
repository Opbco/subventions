...
final class DemandeNoteeAdmin extends AbstractAdmin
{
    private $ts;

    protected $classnameLabel = 'Demandes notées -  ';
    ...
    public function toString(object $object): string
    {
        return $object instanceof Demande
            ? $object->__toString()
            : 'Demande Notée'; // shown in the breadcrumb on the create view
    }
    ...
    protected function configureListFields(ListMapper $list): void
    {
        $list
            ...
            ->add('QuoteFenascoPt', null, ['label' => 'FENASSCO'])
            ->add('cotisationSeducPt', null, ['label' => 'SEDUC'])
            ->add('positionGeoPt', null, ['label' => 'P.G'])
            ->add('apsCnpsPt', null, ['label' => 'CNPS'])
            ->add('reverseRetenuFiscPt', null, ['label' => 'Fiscalité'])
            ->add('percentExamenPt', null, ['label' => 'Exam'])
            ->add('personnelsPt', null, ['label' => 'Personnels (A+C)'])
            ->add('permaVacatairePt', null, ['label' => 'Ratio P/V'])
            ->add('conformitePt', null, ['label' => 'C.I'])
            ->add('equipementsPt', null, ['label' => 'Didactique'])
            ->add('mesuresBarieresPt', null, ['label' => 'Mesures B'])
            ->add('cleanSchoolPt', null, ['label' => 'Clean School'])
            ->add('digitalisationPt', null, ['label' => 'Digitalisation'])
            ->add('score', null, ['label' => 'Note'])
            ->add('user_marking', null, ['label' => 'Examinateur']);
            if ($this->hasAccess('edit')) {
                $list->add('statut', FieldDescriptionInterface::TYPE_CHOICE, [
                    'editable' => true,
                    'choices' => [
                        2 => 'Acceptée',
                        3 => 'Validée',
    ...
    protected function configureFormFields(FormMapper $form): void
    {
        ...
        $form->tab('Détails sur la Demande')
            ...
            if($type == 'Etablissement'){    
                $form->with("Contribution à la réalisation de l'éducation  (08pts)", ['class' => 'col-md-12'])
                ->add('nombreEleve', null, ['label' => "Nombre d'élèves (1pt)", 'disabled' => true, "help"=>"ESG: minimun de 50 en zone rurale et 100 en zone urbaine. EN: minimum de 20"])
                ->add('quoteFenasco', null, ['label' => "Quotes-parts FENASCO pour l'année encours (1pt)", 'disabled' => true])
                ->add('assuranceEleve', null, ['label' => "Assurance des élèves pour l'année encours (1pt)", 'disabled' => true])
                ->add('cotisationSeduc', null, ['label' => "Quotes-parts SEDUC pour l'année encours (1pt)", 'disabled' => true])
                ...
                ->add('apsCnps', null, ['label' => "Cotisations sociales des personnels permanents (1pt)", 'disabled' => true])
                ->add('reverseRetenuFisc', null, ['label' => "Reversement des retenues fiscales (1pt)", 'disabled' => true])
                ->end()
                ->with("Performances de l'établissement (3pts)", ['class' => 'col-md-12'])
                ->add('percentExamen', null, ['label' => "Pourcentage de réussite aux examens officiels", 'disabled' => true, "help"=>"NB: - [30% - 50%[ (1pt) - [50% - 70%[ (2pts) - [70% - 100%[ (3pts)"])
                ->end()
                ->with("Ressources humaines (4pts)", ['class' => 'col-md-12'])
                ->add('personnel_autorise', null, ['label' => "Le personnel enseignant dispose d'une autorisation", 'disabled' => true])
                ->add('personnel_contract', null, ['label' => "Le personnel enseignant dispose d'un contrat de travail", 'disabled' => true])
                ->add('nombrePersonnelEnseignant', null, ['label' => "Nombre de personnel enseignant", 'disabled' => true])
                ->add('nombrePersonnelPermanent', null, ['label' => "Nombre de personnel enseignant permanent", 'disabled' => true])
                ...
                ->add('nombreSalleSuffisant', null, ['label' => "La structure dispose de salle de classes suffisantes et adaptées (1pt)", 'disabled' => true])
                ->add('existAtelier', null, ['label' => "La structure dispose d'ateliers pratiques (1pt)", 'disabled' => true])
                ->add('laboSalleSpecialisee', ChoiceType::class, array('choices' => [
                    'Pas du tout (0pt)' => 0,
                    'Un peu (1pt)' => 1,
                    'Assez (2pts)' => 2
                ], 'label' => 'La structure possède de salles spécialisées en quantité et qualité (2pts)', 'disabled' => true))
                ->add('materielDidactique', null, ['label' => "La structure possède le matériel didactique en quantité et qualité requises (1pt)", 'disabled' => true])
                ->add('equipements', ChoiceType::class, array('choices' => [
                    'Aucun (0pt)' => 0,
                    'Un peu (1pt)' => 1,
                    'suffisant (2pts)' => 2,
                    'Assez (3pts)' => 3
                ], 'label' => 'La structure possède des équipements en quantité et qualité (3pts)', 'disabled' => true))
                ->add('mesuresBarieres', null, ['label' => "La structure dispose d'un dispositif mis en place pour les mesures barrières (1pt)", 'disabled' => true])
                ->add('digitalisation', null, ['label' => "La structure est engagée dans la digitalisation des enseignaments (1pt)", 'disabled' => true])
                ->add('cleanSchool', null, ['label' => "La structure met en oeuvre le clean school (1pt)", 'disabled' => true])
                ->end();
            }
            if($type == 'Etablissement'){
                $form->with("Avis et statut", ['class' => 'col-md-12'])
                ->add('avisDres', ChoiceType::class, array('choices' => [
                    'Défavorable (-5pts)' => -5,
                    'Non prononcé (0pt)' => 0,
                    'Favorable (1pt)' => 1
                ], 'label' => 'Avis du DRES  (1pt)', 'disabled' => true))
    ...
    protected function configureRoutes(RouteCollectionInterface $collection): void
    {
        ...
        $collection->clearExcept(['list', 'show', 'edit', 'batch', 'export']);
    }
}
